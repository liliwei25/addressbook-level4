...
`ImageCommandParser` parses the `Index` provided by the user and passes it to a new `ImageCommand`. If `remove` is detected in the input,
the `ProfilePicture` of the selected by the user will be deleted and reset to default.

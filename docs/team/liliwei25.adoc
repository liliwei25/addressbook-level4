...
When the user wants to delete multiple users at once, it is inconvenient to call the `delete` command several times. With the delete multiple enhancement, the user can just call `delete` once and enter all the persons he wants to delete.
...
Provides the user with convenience of searching for a contact's address and reduces the need to open an external browser to search for the address
...
Allows the user to delete and remove the selected tag from all/selected contact in PocketBook. Since the user is able to add a tag, he should be able to remove a tag.
...
Allows the user to add/edit/remove birthday from a selected contact so that the user can keep track of the birthday of his contacts. Also shows a birthday notification when there is a birthday when user opens the app.
...
The user can close the window or open the window from the tray icon. When the window is closed, the app does not exit but is minimized to the system tray. This allows user to have a faster start up and does not need to restart the app all the time.
...
Adds an profile picture to each person and enables user to choose picture for each person. Helps user remember the face of the person.
